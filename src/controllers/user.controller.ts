...
export const getUserList = async(req: Request, res: Response, next : NextFunction) =>{
    try{
        ...
        res.status(400).json({message: "No user found", error: err instanceof Error ? err.message: "Unknown error"})
    }
}
...
export const updateUserDetails = async(req: Request, res: Response, next: NextFunction) =>{
    try{
        ...
        res.status(200).json({ message: "User updated successfully", updatedUser: updatedUserDetail });
    }catch(err){
        res.status(400).json({message: "User not found", error: err instanceof Error? err.message: "Unknown error"})
    }
}
...
export const deleteUser = async(req: Request, res: Response, next: NextFunction) =>{
    try{
        ...
        res.status(200).json({message: "User deleted succesfully", deletedUser: deleteUser});
    }catch(err){
        res.status(400).json({message: "User not found", error: err instanceof Error ? err.message: "Unknown error"})
    }
